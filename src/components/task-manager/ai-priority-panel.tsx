...
/**
 * AI Priority — analyzes every open task assigned to / created by the user
...
export function AiPriorityPanel({
  ...
  // Initial + parent-triggered load (silent refresh); Re-analyze sets its own loading state
  useEffect(() => {
    let alive = true
          ...
          <>
            {source === 'fallback' && (
              <p className="mb-2 text-[11px] font-medium text-amber-700">
                AI is unavailable right now — showing smart deadline-based order.
              </p>
            )}
